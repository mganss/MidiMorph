...
function lap(dim, cost) {
  ...
  var BIG = 10000 * (sum / dim);
  var epsilon = sum / dim / 10000;
  var rowsol = new Int32Array(dim),
    colsol = new Int32Array(dim),
    u = new Float64Array(dim),
    v = new Float64Array(dim);
  var unassignedfound;
  /* row */
  var i, imin, numfree = 0, prvnumfree, f, i0, k, freerow; // *pred, *free
  /* col */
  var j, j1, j2, endofpath, last, low, up; // *collist, *matches
  /* cost */
  var min, h, umin, usubmin, v2; // *d

  var free = new Int32Array(dim); // list of unassigned rows.
  var collist = new Int32Array(dim); // list of columns to be scanned in various ways.
  var matches = new Int32Array(dim); // counts how many times a row could be assigned.
  var d = new Float64Array(dim); // 'cost-distance' in augmenting path calculation.
  var pred = new Int32Array(dim); // row-predecessor of column in augmenting/alternating path.
